...
def plot_samples(samples, title="design variables", **kwargs):
    ...
    for i in range(imax):
        for j in range(jmax):
            ...
            mu = arr.mean()
            if sigma > 0:
                axh.plot(bins, 1 / (sigma * np.sqrt(2 * np.pi)) * np.exp(-((bins - mu) ** 2) / (2 * sigma ** 2)), linewidth=2, color="r", linestyle="--", alpha=0.5, label="ML approx.")
...
if __name__ == "__main__":
    ...
    desvar_dict = {"m1_E": {"means": np.array([0, 0, 0]), "stdvs": np.array([1, 1, 1])}, "m1_rho": {"means": np.array([0]), "stdvs": np.array([1])}}
