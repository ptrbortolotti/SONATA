...
def plot_fandiagram(res, Omega, RPM_vec, ptype="mean", **kwargs):
    ...
    for i in range(1, 9):
        ...
        string = r"$%i\Omega$" % (i)
        plt.text(x[-1] - 0.06, i * y[-1] + 0.5, string, color="black")
    ...
    if ptype == "mean":
        ...
        if isinstance(sigma, (np.ndarray)):
            for i, d in enumerate(res[:, : len(ref_str)].T):
                ...
                plt.fill_between(
                    x, d - sigma[:, i], d + sigma[:, i], alpha=0.6, edgecolor="r", linestyle=":", facecolor="r", antialiased=True,
                )
        ...
        for i, d in enumerate(res[:, : len(ref_str)].T):
            ...
            if "f" in s:
                ...
                string = r"%s flap" % (s[-1])
                plt.text(x[-1] + 0.01, d[-1], string, color=colorhex)
            elif "l" in s:
                colorhex = "red"
...
def plot_eigenmodes(eigv, string="BLADE_BP_CG01", **kwargs):
    ...
    plt.legend(
        handles=[line1, line2, line3, line4, line5, line6, line7], loc="lower center", ncol=4, bbox_to_anchor=(0.5, -0.7),
    )
    plt.ylim([-1.05, 1.05])
