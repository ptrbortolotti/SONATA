...
from OCC.Core.Geom import Geom_BezierCurve, Geom_TrimmedCurve
...
#                # in obj.Curve:
        ...
        if close_sections:
            ...
            if crv.IsClosed() is False:
                ...
                top = crv.Value(0)
                bot = crv.Value(1)
                mid = gp_Pnt( 0.5*(top.X()+bot.X()), 0.5*(top.Y()+bot.Y()), 0.5*(top.Z()+bot.Z()))
                TE_1 = make_edge(mid, top)
                TE_2 = make_edge(bot, mid)
                edges.append(TE_1)
                edges.append(make_edge(obj))
                edges.append(TE_2)
            else:
                edges.append(make_edge(Geom_TrimmedCurve(obj,0,0.01,False)))
                edges.append(make_edge(Geom_TrimmedCurve(obj,0.01,0.99,False)))
                edges.append(make_edge(Geom_TrimmedCurve(obj,0.99,1.0,False)))
