...
from jobs.RFeil.utls.write_sonata2beamdyn import write_beamdyn_axis, write_beamdyn_prop
...
def beam_struct_eval(flags_dict, cs_pos, job, folder_str, job_str):
    ...
    if flags_dict['flag_run_vabs']:
        ...
        vabs_beam_mass_center_init = np.zeros([len(cs_pos), 2])
        vabs_beam_neutral_axes_init = np.zeros([len(cs_pos), 2])
        vabs_beam_geometric_center_init = np.zeros([len(cs_pos), 2])
        vabs_beam_shear_center_init = np.zeros([len(cs_pos), 2])
        ...
        for i in range(len(job.beam_properties)):
            if job.beam_properties[i, 1] is not None:
                vabs_beam_section_mass[i] = job.beam_properties[i, 1].m00  # mass per unit span
                vabs_beam_mass_center_init[i] = np.array(job.beam_properties[i, 1].Xm[:])  # Center of gravity (mass center)
                vabs_beam_neutral_axes_init[i] = np.array(job.beam_properties[i, 1].Xt[:])  # Neutral axes (tension center)
                vabs_beam_geometric_center_init[i] = np.array(job.beam_properties[i, 1].Xg[:])  # Geometric center
                vabs_beam_shear_center_init[i] = np.array(job.beam_properties[i, 1].Xs[:])  # Generalized Shear Center of the Cross Section
                for j in range(6):
        ...
        if flags_dict['flag_DeamDyn_def_transform']:
            ...
            for n_sec in range(len(cs_pos)):
                ...
                vabs_beam_mass_center[n_sec, :] = [-vabs_beam_mass_center_init[n_sec,0], vabs_beam_mass_center_init[n_sec,1]]
                vabs_beam_neutral_axes[n_sec, :] = [-vabs_beam_neutral_axes_init[n_sec,0], vabs_beam_neutral_axes_init[n_sec,1]]
                vabs_beam_geometric_center[n_sec, :] = [-vabs_beam_geometric_center_init[n_sec,0], vabs_beam_geometric_center_init[n_sec,1]]
                vabs_beam_shear_center[n_sec, :] = [-vabs_beam_shear_center_init[n_sec,0], vabs_beam_shear_center_init[n_sec,1]]
            ...
            vabs_beam_mass_center = vabs_beam_mass_center_init
            vabs_beam_neutral_axes = vabs_beam_neutral_axes_init
            vabs_beam_geometric_center = vabs_beam_geometric_center_init
            vabs_beam_shear_center = vabs_beam_shear_center_init
        ...
        if flags_dict['flag_csv_export']:
            ...
            export_beam_struct_properties(folder_str, job_str, cs_pos, coordsys=coordsys, solver='vabs', beam_stiff=vabs_beam_stiff, beam_inertia=vabs_beam_inertia,
                                          beam_mass_per_length=vabs_beam_section_mass, beam_mass_center=vabs_beam_mass_center,
                                          beam_neutral_axes=vabs_beam_neutral_axes, beam_geometric_center=vabs_beam_geometric_center,
                                          beam_shear_center=vabs_beam_shear_center)
        ...
        if flags_dict['flag_plot_struct_characteristics']:
            # plot inertia matrix
            save_path = [folder_str + job_str[0:-5] + '_VABS_mass_matrix' + str_ext + '.png']
            fig_title = 'Mass matrix'
            plot_beam_props_6by6(cs_pos, vabs_beam_inertia, fig_title, save_path)
            # plot stiffness matrix
            save_path = [folder_str + job_str[0:-5] + '_VABS_stiffness_matrix' + str_ext + '.png']
            fig_title = 'Stiffness matrix'
            plot_beam_props_6by6(cs_pos, vabs_beam_stiff, fig_title, save_path)
            # plot axes locations
            save_path = [folder_str + job_str[0:-5] + '_VABS_axes_locations' + str_ext + '.png']
            plot_beam_axes(cs_pos, vabs_beam_mass_center, vabs_beam_neutral_axes, vabs_beam_geometric_center, vabs_beam_shear_center, save_path)
