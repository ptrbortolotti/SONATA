...
# from SONATA.cbm.topo.utils import Topo
#from OCC.Core.U  Topology import Topo
...
class STEPImporter(object):
    ...
    def set_filename(self, filename):
        if not os.path.isfile(filename):
            print
            "STEPImporter initialization Error: file %s not found." % filename
            self._filename = None
        else:
    ...
    def read_file(self):
        ...
        if not self._filename:
            print
            "ReadFile Error: first set the filename."
            return False
        ...
        if status == IFSelect_RetDone:
            ...
            for n in range(1, nbr + 1):
                ...
                if self.nbs == 0:
                    print
                    "At least one shape in STEP cannot be transfered"
            ...
            print
            "Error: can't read file %s" % self._filename
            return False
        return False
...
class STEPOCAF_Import(object):
    ...
    def read_file(self):
        ...
        print
        'Number of shapes at root :%i' % Labels.Length()
        for i in range(Labels.Length()):
            sub_shapes_labels = TDF_LabelSequence()
            print
            "Is Assembly?", shape_tool.isassembly(Labels.Value(i + 1))
            sub_shapes = shape_tool.getsubshapes(Labels.Value(i + 1), sub_shapes_labels)
            print
            'Number of subshapes in the assemly :%i' % sub_shapes_labels.Length()
        l_Colors.GetObject().GetColors(ColorLabels)
        print
        'Number of colors=%i' % ColorLabels.Length()
        # if(CL_Len>0):
        ...
        for i in range(Labels.Length()):
            ...
            print
            dir(h_shape_tool.GetObject())
            # print c
            print
            "Lenght of popo :%i" % m.GetObject().Length()
...
class StepOCAF_Export(object):
    ...
    def add_shape(self, shape, color=None, layer=None):
        ...
        '''
        assert issubclass(shape.__class__, TopoDS_Shape) or isinstance(shape,
                                                                       TopoDS_Shape), 'not a TopoDS_Shape or subclass'
