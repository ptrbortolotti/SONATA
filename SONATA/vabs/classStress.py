...
class Stress(object):
    ...
    def __getTensor(self): return self.__tensor
    def __setTensor(self,x): 
        if isinstance(x, np.ndarray) and x.shape == (3,3):
                self.__tensor = x
        else: raise TensorException
    ...
    def sigma_vM(self):
        ...
        sigma_vM = math.sqrt(self.sigma11**2 + self.sigma22**2 + self.sigma33**2 \
                           - self.sigma11*self.sigma22 - self.sigma11*self.sigma33 - self.sigma22*self.sigma33 \
