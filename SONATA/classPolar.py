    ...
    def _check_coefficients(self, x):
        ...
        if isinstance(x,np.ndarray) and x.shape[1] == 2: 
            if all((x[:,0] >= -np.pi-1e-4) & (x[:,0] <= np.pi+1e-4)):
                ...
                return False
                print('WARNING: Attribute not changed; Polar.coefficients' +
                      ' AoA values have to be between ~ [-pi,+pi] ')
        else: 
            return False
            print('WARNING: Attribute not changed; Polar.coefficients has' +
                    ' to be a np.array with shape ( ... , 2)')
